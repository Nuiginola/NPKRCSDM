...
beam_name = st.text_input("รหัสคาน (Beam No.)", value="B-01")
...
with col2:
    ...
    L_m = st.number_input("ช่วงคาน L (m)", value=4.0, step=0.1)
    ...
    line_ll = st.number_input("Line Load, LL (kg/m)", value=300.0, step=10.0)

with col3:
    st.subheader("น้ำหนักจุด (Point Loads)")
    n_points = st.number_input("จำนวนจุด Point Load", min_value=0, max_value=10, value=0, step=1)
    point_loads_input = []
    for i in range(int(n_points)):
        st.markdown(f"**จุดที่ {i + 1}**")
        pc1, pc2, pc3 = st.columns(3)
        with pc1:
            p_dl = st.number_input(f"P_DL (kg)", value=0.0, step=50.0, key=f"beam_pl_dl_{i}")
        with pc2:
            p_ll = st.number_input(f"P_LL (kg)", value=0.0, step=50.0, key=f"beam_pl_ll_{i}")
        with pc3:
            p_x = st.number_input(f"ระยะ x (m)", value=0.0, step=0.1, key=f"beam_pl_x_{i}")
...
if "beam_result" in st.session_state:
    ...
    st.image(section_png, caption="Beam Section")
