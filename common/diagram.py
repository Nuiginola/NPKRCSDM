...
def draw_ow_plan_png(main_bar_dia_mm: float, main_bar_spacing_cm: float,
    ...
    # แถบเหล็กบน (negative-moment top steel) ที่ขอบรองรับ — ไฮไลท์แถบสีเหลือง + tick เหล็กบน
    # + highlight ขอบเขตพื้นด้านที่ต่อเนื่อง (เส้นเขียวเข้ม) แบบเดียวกับพื้นสองทาง (1.3)
    BAND_FILL = "#f4d9a0"       # สีแถบเหล็กบน (เหลืองอ่อน) เท่ากับ BAND_FILL ของ draw_tw_plan_png
    HL_COLOR = "#2E7D32"        # สี highlight ขอบต่อเนื่อง (เขียวเข้ม) เท่ากับพื้นสองทาง
    HL_OFF = 0.05
    BAND_DEPTH = 0.32           # ความลึกแถบเหล็กบนที่วาด (~S/4 ในสเกลภาพ)

    def _ow_top_band(from_top):
        """วาดแถบเหล็กบนที่ขอบรองรับ (บน=from_top True / ล่าง=False): แถบเหลือง + tick เหล็กบน
        (แนว S ยื่นจากขอบเข้ามา) + เส้นเขียวเข้ม highlight ขอบเขตพื้นด้านที่ต่อเนื่อง"""
        y0 = (H - BAND_DEPTH) if from_top else 0.0
        ax.add_patch(patches.Rectangle((0, y0), W, BAND_DEPTH, facecolor=BAND_FILL,
                                        edgecolor=LINE_COLOR, linewidth=1.0, zorder=2))
        y_edge = H if from_top else 0.0
        y_in = y0 if from_top else y0 + BAND_DEPTH
        for f in (0.12, 0.28, 0.44, 0.56, 0.72, 0.88):
            x = W * f
            ax.plot([x, x], [y_edge, y_in], color=LINE_COLOR, linewidth=0.9, zorder=4)
        y_hl = (H + HL_OFF) if from_top else -HL_OFF
        ax.plot([0, W], [y_hl, y_hl], color=HL_COLOR, linewidth=5.0,
...
def concrete_Ec_ksc(fc_ksc: float) -> float:
    """โมดูลัสยืดหยุ่นของคอนกรีต Ec = 15100·√f'c (ksc = kg/cm²) — สูตรมาตรฐาน ACI/มยผ.
    สำหรับคอนกรีตน้ำหนักปกติ (ค่าที่โปรแกรมนี้ใช้ทุกโมดูล)."""
    return 15100.0 * math.sqrt(max(fc_ksc, 1.0))


def gross_EI_kgcm2(b_cm: float, h_cm: float, fc_ksc: float) -> float:
    """ความแข็งเกร็งดัด EI ของหน้าตัดเต็ม (gross, ไม่หักรอยแตก):
    Ec = 15100√f'c (ksc), Ig = b·h³/12 (cm⁴) → EI (kg·cm²)."""
    Ig = b_cm * h_cm ** 3 / 12.0
    return concrete_Ec_ksc(fc_ksc) * Ig


def compute_deflection_mm(x_arr: list, m_arr: list, EI_kgcm2: float,
                          support_xs: list = None, cantilever_from_left: bool = False):
    """คำนวณการโก่งตัว (elastic curve) จากไดอะแกรมโมเมนต์ ด้วยการอินทิเกรต M/EI สองครั้ง
    (โมเมนต์-พื้นที่/double integration). หน่วยเข้า: x_arr เป็น m, m_arr เป็น kg·m (sagging บวก),
    EI_kgcm2 เป็น kg·cm². เงื่อนไขขอบ:
      - cantilever_from_left=True : ยึดแน่นที่ปลายซ้าย x_arr[0] → y=0 และ θ=0 ที่นั่น (a=b=0)
      - อื่นๆ : y=0 ที่ตำแหน่งจุดรองรับ support_xs (fit rigid body a+b·x ด้วย least-squares —
        ผ่านจุดรองรับริม 2 จุดพอดี, จุดรองรับภายในของคานต่อเนื่อง ≈0 โดยอัตโนมัติจากฟิสิกส์)
    คืน (y_mm_list, y_max_abs_mm, y_max_x_m) โดย y เป็นลบ = โก่งลง (v''=M/EI, v บวก=ขึ้น)"""
    import numpy as np
    n = len(x_arr)
    if EI_kgcm2 <= 0 or n < 3:
        return [0.0] * n, 0.0, 0.0
    x = np.asarray(x_arr, dtype=float) * 100.0     # m -> cm
    M = np.asarray(m_arr, dtype=float) * 100.0     # kg·m -> kg·cm
    kappa = M / EI_kgcm2                            # ความโค้ง (1/cm)

    def _cumtrapz(y, xv):
        out = np.zeros_like(y)
        d = np.diff(xv)
        out[1:] = np.cumsum((y[:-1] + y[1:]) / 2.0 * d)
        return out

    theta_p = _cumtrapz(kappa, x)                   # slope (rad), theta_p[0]=0
    y_p = _cumtrapz(theta_p, x)                     # deflection (cm), y_p[0]=0
    if cantilever_from_left or not support_xs:
        y = y_p                                     # a=b=0 (ยึดแน่นปลายซ้าย)
    else:
        sx = np.asarray(support_xs, dtype=float) * 100.0
        yp_at = np.interp(sx, x, y_p)
        A = np.vstack([np.ones_like(sx), sx]).T     # fit y_p + a + b·x = 0 ที่จุดรองรับ
        coef, *_ = np.linalg.lstsq(A, -yp_at, rcond=None)
        y = y_p + coef[0] + coef[1] * x
    y_mm = y * 10.0                                 # cm -> mm
    idx = int(np.argmax(np.abs(y_mm)))
    return y_mm.tolist(), float(abs(y_mm[idx])), float(x[idx] / 100.0)


def effective_Ie_cm4(b_cm, h_cm, d_cm, fc_ksc, As_cm2, Ma_service_kgm, As_comp_cm2=0.0):
    """โมเมนต์ความเฉื่อยประสิทธิผล Ie (หน้าตัดร้าว) ตามสูตร Branson (ACI 318):
      Mcr = fr·Ig/yt,  fr = 2√f'c (ksc),  yt = h/2
      Icr = b·c³/3 + n·As·(d−c)²   (n=Es/Ec, c=kd, k=√((nρ)²+2nρ)−nρ)
      Ie = (Mcr/Ma)³·Ig + [1−(Mcr/Ma)³]·Icr ≤ Ig  (ถ้า Ma≤Mcr → ยังไม่ร้าว = Ig)
    คืน (Ie_cm4, Icr_cm4, Mcr_kgm, Ec_ksc, n)."""
    Ig = b_cm * h_cm ** 3 / 12.0
    Ec = concrete_Ec_ksc(fc_ksc)
    fr = 2.0 * math.sqrt(max(fc_ksc, 1.0))          # modulus of rupture (ksc) ≈ 2√f'c
    yt = h_cm / 2.0
    Mcr_kgm = (fr * Ig / yt) / 100.0                # kg·cm → kg·m
    Es = 2.04e6                                     # โมดูลัสเหล็ก (ksc)
    n = Es / Ec if Ec > 0 else 8.0
    if As_cm2 > 0 and d_cm > 0:
        rho = As_cm2 / (b_cm * d_cm)
        k = math.sqrt((n * rho) ** 2 + 2 * n * rho) - n * rho
        c = k * d_cm
        Icr = b_cm * c ** 3 / 3.0 + n * As_cm2 * (d_cm - c) ** 2
    else:
        Icr = Ig
    Ma = abs(Ma_service_kgm)
    if Ma <= Mcr_kgm or Ma <= 0:
        Ie = Ig                                     # ยังไม่ร้าว
    else:
        r3 = (Mcr_kgm / Ma) ** 3
        Ie = min(r3 * Ig + (1.0 - r3) * Icr, Ig)
    return Ie, Icr, Mcr_kgm, Ec, n


def compute_service_deflection(x_arr, m_arr_factored, b_cm, h_cm, d_cm, fc_ksc,
                               As_cm2, As_comp_cm2, Ma_factored_kgm,
                               r_service, sustained_frac, span_L_m,
                               support_xs=None, cantilever=False, xi=2.0, allow_ratio=360.0):
    """คำนวณการโก่งตัวตามมาตรฐาน (Service + Ie + Long-term):
      - โมเมนต์ใช้งาน = r_service × โมเมนต์ factored  (r = โหลด service/factored)
      - Ie (Branson) จาก Ma ใช้งาน
      - โก่งทันที (immediate) = ∫∫ M_service/(Ec·Ie)
      - โก่งระยะยาว = λΔ·(โก่งจากโหลดคงค้าง),  λΔ = ξ/(1+50ρ′),  ρ′=As′/(bd)
        โก่งจากโหลดคงค้าง = โก่งทันที × sustained_frac (สัดส่วนโหลดคงค้าง DL/รวม)
      - โก่งรวม = immediate + long-term  (เส้นโค้งรวม = เส้นโก่งทันที × (1+λΔ·sustained))
    คืน dict ค่าทั้งหมด (รวม total_arr สำหรับวาดกราฟ)."""
    Ie, Icr, Mcr_kgm, Ec, n = effective_Ie_cm4(
        b_cm, h_cm, d_cm, fc_ksc, As_cm2, r_service * Ma_factored_kgm, As_comp_cm2)
    Ig = b_cm * h_cm ** 3 / 12.0
    EI_eff = Ec * Ie
    m_service = [r_service * m for m in m_arr_factored]
    imm_arr, imm_max, imm_x = compute_deflection_mm(
        x_arr, m_service, EI_eff, support_xs=support_xs, cantilever_from_left=cantilever)
    rho_c = (As_comp_cm2 / (b_cm * d_cm)) if d_cm > 0 else 0.0
    lam = xi / (1.0 + 50.0 * rho_c)
    factor = 1.0 + lam * sustained_frac
    total_arr = [d * factor for d in imm_arr]
    total_max = imm_max * factor
    lt_max = imm_max * (factor - 1.0)
    allow_mm = span_L_m * 1000.0 / allow_ratio
    return {
        "Ig": Ig, "Icr": Icr, "Ie": Ie, "Ec": Ec, "n": n, "fr": 2.0 * math.sqrt(max(fc_ksc, 1.0)),
        "Mcr_kgm": Mcr_kgm, "Ma_service_kgm": abs(r_service * Ma_factored_kgm),
        "lambda_delta": lam, "rho_comp": rho_c, "r_service": r_service, "sustained_frac": sustained_frac,
        "imm_arr": imm_arr, "imm_max": imm_max, "imm_x": imm_x,
        "total_arr": total_arr, "total_max": total_max, "lt_max": lt_max,
        "allow_mm": allow_mm, "allow_ratio": allow_ratio, "ok": total_max <= allow_mm,
        "span_L_m": span_L_m,
    }


def build_deflection_calc_section(pkg, b_cm, h_cm, fc_ksc):
    """สร้าง section แผ่นคำนวณ 'การตรวจสอบการโก่งตัว' จากผลลัพธ์ compute_service_deflection (pkg).
    เป็นฟังก์ชันข้อมูลล้วน (ไม่มี st.*) ใช้ร่วมทั้ง 3 โมดูลคาน."""
    cracked = pkg["Ie"] < pkg["Ig"] - 1e-6
    steps = [
        {"desc": "โมดูลัสยืดหยุ่นคอนกรีต", "formula": "E<sub>c</sub> = 15100√f′<sub>c</sub>",
         "sub": f"15100√{fc_ksc:.0f}", "result": f"{pkg['Ec']:,.0f} ksc"},
        {"desc": "โมเมนต์ความเฉื่อยหน้าตัดเต็ม / โมเมนต์แตกร้าว",
         "formula": "I<sub>g</sub> = bh³/12 ; M<sub>cr</sub> = f<sub>r</sub>I<sub>g</sub>/y<sub>t</sub> (f<sub>r</sub>=2√f′<sub>c</sub>)",
         "sub": f"I<sub>g</sub>={pkg['Ig']:,.0f} cm⁴", "result": f"M<sub>cr</sub> = {pkg['Mcr_kgm']:,.0f} kg·m"},
        {"desc": "โมเมนต์ใช้งาน (service) ที่หน้าตัดวิกฤต",
         "formula": "M<sub>a</sub> = (โหลด service/factored) × M<sub>u</sub>",
         "sub": f"อัตราส่วนโหลด r = {pkg['r_service']:.3f}", "result": f"M<sub>a</sub> = {pkg['Ma_service_kgm']:,.0f} kg·m"},
        {"desc": ("โมเมนต์ความเฉื่อยประสิทธิผล (Branson) — หน้าตัดร้าว" if cracked
                  else "โมเมนต์ความเฉื่อยประสิทธิผล — ยังไม่ร้าว (M<sub>a</sub> ≤ M<sub>cr</sub>)"),
         "formula": ("I<sub>e</sub> = (M<sub>cr</sub>/M<sub>a</sub>)³I<sub>g</sub> + [1−(M<sub>cr</sub>/M<sub>a</sub>)³]I<sub>cr</sub>"
                     if cracked else "I<sub>e</sub> = I<sub>g</sub>"),
         "sub": (f"I<sub>cr</sub>={pkg['Icr']:,.0f}, n={pkg['n']:.1f}" if cracked else ""),
         "result": f"I<sub>e</sub> = {pkg['Ie']:,.0f} cm⁴"},
        {"desc": "การโก่งตัวทันที (Immediate) จาก M/E<sub>c</sub>I<sub>e</sub> (โหลด service)",
         "formula": "δ<sub>i</sub> = ∫∫ (M<sub>service</sub>/E<sub>c</sub>I<sub>e</sub>) dx",
         "result": f"{pkg['imm_max']:.2f} mm ที่ x={pkg['imm_x']:.2f} m"},
        {"desc": "การโก่งตัวระยะยาว (Long-term: creep+shrinkage)",
         "formula": "δ<sub>lt</sub> = λ<sub>Δ</sub>·δ<sub>sustained</sub> , λ<sub>Δ</sub> = ξ/(1+50ρ′)",
         "sub": f"λ<sub>Δ</sub>={pkg['lambda_delta']:.2f} (ξ=2.0, ρ′={pkg['rho_comp']:.4f}), โหลดคงค้าง {pkg['sustained_frac']*100:.0f}%",
         "result": f"{pkg['lt_max']:.2f} mm"},
        {"desc": "การโก่งตัวรวม (Total = Immediate + Long-term)",
         "formula": "δ<sub>total</sub> = δ<sub>i</sub> + δ<sub>lt</sub>",
         "result": f"{pkg['total_max']:.2f} mm"},
        {"desc": "การโก่งตัวที่ยอมให้",
         "formula": f"δ<sub>allow</sub> = L/{pkg['allow_ratio']:.0f}",
         "sub": f"{pkg['span_L_m']*100:.0f}/{pkg['allow_ratio']:.0f} cm", "result": f"{pkg['allow_mm']:.2f} mm"},
        {"desc": "ตรวจสอบ",
         "formula": f"δ<sub>total</sub> = {pkg['total_max']:.2f} {'≤' if pkg['ok'] else '>'} δ<sub>allow</sub> = {pkg['allow_mm']:.2f} mm",
         "result": "ผ่าน ✓" if pkg["ok"] else "ไม่ผ่าน ✗",
         "note": "ตามมาตรฐาน ACI 318: โหลดใช้งาน (service) + หน้าตัดร้าว I<sub>e</sub> (Branson) + การโก่งระยะยาว λ<sub>Δ</sub>"},
    ]
    return {"title": "การตรวจสอบการโก่งตัว (Deflection — Service + Ie + Long-term)", "steps": steps}


def _draw_deflection_panel(ax, x_arr, defl_mm, xlabel, defl_max_mm=None, defl_max_x_m=None):
    """วาดแผง Deflection Diagram (Elastic Curve) — สไตล์เดียวกับ SFD/BMD (สีม่วง #6a1b9a)
    y เป็น mm (ลบ=โก่งลง) + callout δmax ที่จุดโก่งมากสุด."""
    ax.plot(x_arr, defl_mm, color="#6a1b9a", linewidth=1.6)
    ax.fill_between(x_arr, defl_mm, 0, color="#6a1b9a", alpha=0.13)
    ax.axhline(0, color="black", linewidth=0.9)
    ax.set_title("Deflection Diagram (Elastic Curve)", fontsize=11.5, fontweight="bold", color=CAPTION_COLOR)
    ax.set_xlabel(xlabel, fontsize=9.5)
    ax.set_ylabel("δ (mm.)", fontsize=9.5)
    ax.grid(True, linewidth=0.4, alpha=0.5)
    # ค่าโก่งเป็น mm (ค่าน้อย) — ใช้ formatter ทศนิยม 2 ตำแหน่งพร้อมเครื่องหมาย (ไม่ใช้แบบจำนวนเต็ม
    # ของ SFD/BMD ที่จะปัดเป็น "-0")
    def _defl_tick_fmt(v, pos):
        return "0" if abs(v) < 1e-9 else f"{v:+.2f}"
    ax.yaxis.set_major_formatter(FuncFormatter(_defl_tick_fmt))
    d_min, d_max = min(defl_mm), max(defl_mm)
    if defl_max_mm is None:
        _idx = max(range(len(defl_mm)), key=lambda i: abs(defl_mm[i]))
        defl_max_mm, defl_max_x_m = abs(defl_mm[_idx]), x_arr[_idx]
    _peak_y = defl_mm[min(range(len(defl_mm)), key=lambda i: defl_mm[i])]
    ax.annotate(f"δmax = {defl_max_mm:.2f} mm.\n(at x={defl_max_x_m:.2f} m.)",
                xy=(defl_max_x_m, _peak_y), xytext=(0, -20),
                textcoords="offset points", fontsize=9, color="#6a1b9a", ha="center", fontweight="bold",
                arrowprops=dict(arrowstyle="-", color="#6a1b9a", linewidth=0.8))
    d_span = max(d_max - d_min, 1e-6)
    ax.set_ylim(d_min - 0.42 * d_span, d_max + 0.20 * d_span)


def draw_beam_sfd_bmd_png(x_arr: list, v_arr: list, m_arr: list, L_m: float,
                           vu_max_kg: float, mu_max_kg_m: float, mu_max_x_m: float,
                           defl_mm: list = None, defl_max_mm: float = None,
                           defl_max_x_m: float = None) -> bytes:
    ...
    defl_mm: ถ้าส่งมา (list การโก่งตัว mm ตรงกับ x_arr) จะเพิ่มแผงที่ 3 "Deflection
    Diagram (Elastic Curve)" ต่อจาก BMD ในตำแหน่ง/สัดส่วนเดียวกัน."""
    _has_defl = defl_mm is not None
    if _has_defl:
        fig, (ax_v, ax_m, ax_d) = plt.subplots(3, 1, figsize=(9.5, 10.2), dpi=150)
    else:
    ...
    _axes = [ax_v, ax_m]
    if _has_defl:
        _draw_deflection_panel(ax_d, x_arr, defl_mm, "Distance from left support, x (m.)",
                               defl_max_mm, defl_max_x_m)
        _axes.append(ax_d)
    for ax in _axes:
...
def draw_beam_section_png(b_cm: float, h_cm: float,
                           ...
                           cover_cm: float = 3.0, torsion=None,
                           bot_dia_mm: float = None, top_dia_mm: float = None,
                           bot_label_lines=None, top_label_lines=None,
                           hide_bar_leader: bool = False) -> bytes:
    ...
    # ขนาดเหล็กบน/ล่าง แยกกันได้ (โหมดจัดเหล็กเอง) — ถ้าไม่ส่งมาใช้ main_bar_dia_mm (เข้ากันได้กับของเดิม)
    _bot_dia = bot_dia_mm if bot_dia_mm else main_bar_dia_mm
    _top_dia = top_dia_mm if top_dia_mm else main_bar_dia_mm
    bot_r_m = _bot_dia / 2000.0
    top_r_m = _top_dia / 2000.0
    bar_r_m = main_bar_dia_mm / 2000.0   # ใช้กับเหล็กยืนรับแรงบิด A_l (ด้านข้าง)
    edge = cover_m + stirrup_r_m
    bot_pitch_m = (_bot_dia / 10.0 + 2.5) / 100.0   # ระยะห่างชั้น เท่ากับ LAYER_CLEAR_SPACING_CM ของ engine
    top_pitch_m = (_top_dia / 10.0 + 2.5) / 100.0

    def _row(n: int, y: float, r: float):
        if n <= 0:
            return []
        if n == 1:
            xs = [b_m / 2.0]
        else:
            usable = b_m - 2.0 * (edge + r)
            xs = [edge + r + usable * i / (n - 1) for i in range(n)]
        for x in xs:
            ax.add_patch(patches.Circle((x, y), r, facecolor="black", edgecolor="black", zorder=5))
        return xs
    ...
    def _bar_label(total: int, layers: list, side: str, dia: float) -> str:
        base = f"{total}-{main_bar_type}{dia:.0f} ({side})"
        if len(layers) <= 1:
            return base
    ...
    # --- labels / leaders ---
    # hide_bar_leader=True → ไม่วาดเส้นชี้ (arrow) ; bot/top_label_lines → ใช้ป้ายกำหนดเอง
    # (โหมดจัดเหล็กเอง: แยกเหล็กหลัก/พิเศษคนละบรรทัด + ต่อท้าย "Ext.")
    _arrow_bot = None if hide_bar_leader else dict(arrowstyle="-", color=LINE_COLOR, linewidth=0.8)
    if xs_bot:
        _lbl_bot = "\n".join(bot_label_lines) if bot_label_lines else _bar_label(total_bottom, bottom_layers, "Bottom", _bot_dia)
        ax.annotate(_lbl_bot, xy=(xs_bot[-1], y_bot_leader), xytext=(b_m + 0.06, y_bottom0),
                    fontsize=9, va="center", ha="left", color="black", arrowprops=_arrow_bot)
    if xs_top:
        _lbl_top = "\n".join(top_label_lines) if top_label_lines else _bar_label(total_top, top_layers, "Top", _top_dia)
        ax.annotate(_lbl_top, xy=(xs_top[-1], y_top_leader), xytext=(b_m + 0.06, y_top0),
                    fontsize=9, va="center", ha="left", color="black", arrowprops=_arrow_bot)
    ...
    if _tor_on:
        ...
        # ป้ายนี้อยู่ฝั่งซ้ายเดียวกับเส้นบอกระยะ h (ย้ายมาซ้ายพร้อม h) — ดันออกไปพ้นแนวเส้น h
        # (x=-0.42) กันทับกัน ยิ่งกว่าตำแหน่งเดิมตอน h ยังอยู่ขวา
        ax.annotate(al_lbl, xy=(x_left, side_ys[len(side_ys) // 2]),
                    xytext=(-0.85, h_m * 0.30), fontsize=8.5, va="center", ha="right",
                    color=_TOR_COLOR,
    ...
    # ป้ายเหล็กปลอกอยู่ด้านขวา (สลับด้าน — h อยู่ซ้าย) ลากเส้นชี้จากขอบขวาของกรอบปลอก
    # เส้นชี้สั้นลง (b_m+0.18) + ขยับข้อความเข้ามาใกล้กล่อง กันเส้นไดเมนชั่นยาวเกินไป
    ax.annotate(_stir_txt,
                xy=(b_m - inset, h_m / 2.0), xytext=(b_m + 0.18, h_m / 2.0),
                fontsize=9, va="center", ha="left", color=_stir_edge,
                arrowprops=dict(arrowstyle="-", color=_stir_edge, linewidth=0.8))

    # dimensions — h อยู่ด้านซ้าย (สลับด้านจากเดิมที่เคยอยู่ขวา)
    _dim_h_generic(ax, -0.14, 0, b_m, f"b = {b_cm:.0f} cm.")
    _dim_v_generic(ax, -0.40, 0, h_m, f"h = {h_cm:.0f} cm.")

    ax.text(b_m / 2.0, h_m + 0.10, "Beam Section", fontsize=12, fontweight="bold",
            ha="center", va="bottom", color=CAPTION_COLOR)

    # กรอบภาพ: ตัดขอบว่างส่วนเกินให้เหลือเฉพาะเท่าที่เนื้อหาต้องใช้ เพื่อให้กล่องรูปตัดคาน
    # เป็นสัดส่วนใหญ่ที่สุดของภาพ (ยิ่งขอบว่างมาก กล่องยิ่งดูเล็ก) — ฝั่งซ้ายกว้างขึ้นเฉพาะเมื่อมี
    # ป้ายเหล็กยืนรับแรงบิด (Al) ที่ยื่นออกไปไกล
    _xlim_left = -1.05 if _tor_on else -0.62
    ax.set_xlim(_xlim_left, b_m + 1.00)
    ax.set_ylim(-0.28, h_m + 0.26)
    ax.set_aspect("equal")
    ax.axis("off")
...
def draw_continuous_beam_sfd_bmd_png(result, defl_mm: list = None,
                                     defl_max_mm: float = None, defl_max_x_m: float = None) -> bytes:
    ...
    _has_defl = defl_mm is not None
    if _has_defl:
        fig, (ax_v, ax_m, ax_d) = plt.subplots(3, 1, figsize=(11.5, 11.4), dpi=150)
    else:
    ...
    _axes = [ax_v, ax_m]
    if _has_defl:
        _draw_deflection_panel(ax_d, x_arr, defl_mm, "Distance along beam, x (m.)",
                               defl_max_mm, defl_max_x_m)
        ax_d.set_title("Deflection Diagram (Elastic Curve) — Full Beam", fontsize=11.5,
                       fontweight="bold", color=CAPTION_COLOR)
        _axes.append(ax_d)
...
def draw_cantilever_beam_sfd_bmd_png(result, L_m: float, defl_mm: list = None,
                                     defl_max_mm: float = None, defl_max_x_m: float = None) -> bytes:
    ...
    _axes = [ax_v, ax_m]
    if _has_defl:
        _draw_deflection_panel(ax_d, x_arr, defl_mm, "Distance from support (fixed end), x (m.)",
                               defl_max_mm, defl_max_x_m)
        _axes.append(ax_d)
    for ax in _axes:
        ax.set_xlim(-L_m * 0.03, L_m * 1.03)
