...
<table class="mini-table">
  ...
  <tr><td colspan="2">As2 (เหล็กรับแรงอัดที่ต้องเพิ่ม, สมมติ f<sub>s</sub>&prime;=fy)</td>
    ...
    # --- เงื่อนไขจุดรองรับ (Pin/Fix) + โมเมนต์ปลายยึดแน่น (คานช่วงเดียว) ---
    _lf = getattr(result, "left_fixed", False)
    _rf = getattr(result, "right_fixed", False)
    _mneg = getattr(result, "mu_neg_max_kgm", 0.0)
    _sn = lambda fx: "ยึดแน่น (Fix)" if fx else "ยึดหมุน (Pin)"
    support_row = (f'  <tr><td>เงื่อนไขจุดรองรับ</td>'
                   f'<td class="dv" colspan="2">ปลายซ้าย {_sn(_lf)} &nbsp;—&nbsp; ปลายขวา {_sn(_rf)}</td></tr>')
    end_moment_rows = ""
    if _lf or _rf:
        end_moment_rows = (
            f'  <tr><td>M ที่ปลายซ้าย (Fixed-End Moment)</td>'
            f'<td class="dv">{getattr(result, "m_a_kgm", 0.0):+.2f}</td><td class="dv">kg-m.</td></tr>\n'
            f'  <tr><td>M ที่ปลายขวา (Fixed-End Moment)</td>'
            f'<td class="dv">{getattr(result, "m_b_kgm", 0.0):+.2f}</td><td class="dv">kg-m.</td></tr>')

    if _mneg > 1e-6:
        top_bars_html = f'''<h2>6. เหล็กเสริมบน (Top Bars — ตามโมเมนต์ลบที่ปลายยึดแน่น)</h2>
<table class="mini-table">
  <tr><th>Mu&minus; (kg-m.)</th><th>d (cm.)</th><th>Ru (ksc)</th><th>&rho; ที่ใช้</th><th>As ต้องการ (cm&sup2;)</th></tr>
  <tr><td>{_mneg:.0f}</td><td>{result.top.d_cm:.2f}</td><td>{result.top.ru_ksc:.2f}</td>
  ...
  <tr><td colspan="2">As ที่ใช้จริง</td><td colspan="2">{result.top.as_provided_cm2:.2f} cm&sup2;</td>
      <th class="{ok_class(result.top.reinf_ok)}">{ok_th(result.top.reinf_ok)}</th></tr>
</table>'''
    else:
        top_bars_html = f'''<h2>6. เหล็กเสริมบน (Top Bars — เหล็กยึดเหล็กปลอกขั้นต่ำ)</h2>
<table class="mini-table">
  <tr><td colspan="2">คานช่วงเดียวยึดหมุนทั้งสองด้าน (simply supported) ไม่มีโมเมนต์ลบที่จุดรองรับ —
