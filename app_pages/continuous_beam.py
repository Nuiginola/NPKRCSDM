...
def _build_calc_sections(inp, result):
    """วิธีการคำนวณและสูตรที่ใช้ (คานต่อเนื่อง) — แสดงวิธีของช่วง/จุดรองรับวิกฤต (governing)
    ดึงค่าจาก result (ไม่คำนวณซ้ำ) รายละเอียดครบทุกช่วง/จุดรองรับอยู่ในส่วนตารางด้านล่างแล้ว"""
    b = inp.b_cm
    fc = inp.fc_ksc
    gsp = result.spans[result.governing_span_index]
    gsup = result.supports[result.governing_support_index]

    def _flex_steps(flex, mu_kgm, label, kind):
        d = flex.d_cm
        steps = [
            ...
            {"desc": "สัมประสิทธิ์ต้านทานโมเมนต์ R<sub>u</sub>",
             ...
             "sub": f"{mu_kgm*100:,.0f}/({PHI_B:.2f}×{b:.0f}×{d:.1f}²)",
        ...
        if not flex.doubly_reinforced:
            steps += [
                ...
                {"desc": "พื้นที่เหล็กที่ต้องการ A<sub>s</sub>",
            ...
            steps.append(
                {"desc": "ต้องเสริมเหล็กรับแรงอัด (Doubly-reinforced)",
                 "formula": f"M<sub>u2</sub> = {flex.mu2_kgm:,.0f} kg·m → A<sub>s</sub> = {flex.as_req_cm2:.2f} cm² (A<sub>s2</sub> = {flex.as_comp_req_cm2:.2f})"})
        steps.append(
            {"desc": f"เลือกใช้เหล็ก{kind}",
             "formula": (f"ใช้ {label} → A<sub>s,จัดให้</sub> = {flex.as_provided_cm2:.2f} cm² "
                         f"{'≥' if flex.reinf_ok else '<'} A<sub>s,ต้องการ</sub> = {flex.as_req_cm2:.2f} cm²"),
             "result": "ผ่าน ✓" if flex.reinf_ok else "ไม่ผ่าน ✗"})
        return steps

    analysis = [
        {"desc": "วิธีวิเคราะห์โครงสร้าง",
         "formula": "วิเคราะห์คานต่อเนื่องด้วยสมการสามโมเมนต์ (Three-Moment Equation)",
         "note": "หาโมเมนต์ที่จุดรองรับทุกจุด แล้วหาโมเมนต์บวกสูงสุดในแต่ละช่วง (รายละเอียดครบทุกจุดในตารางด้านล่าง)"},
        {"desc": f"จุดรองรับวิกฤต (โมเมนต์ลบสูงสุด): S{gsup.index}",
         "formula": "M<sub>u,รองรับ</sub> (hogging)", "result": f"{abs(gsup.moment_kgm):,.0f} kg·m"},
        {"desc": f"ช่วงวิกฤต (โมเมนต์บวกสูงสุด): ช่วงที่ {result.governing_span_index + 1}",
         "formula": "M<sub>u,ช่วง</sub> (sagging)",
         "result": f"{gsp.mu_pos_max_kgm:,.0f} kg·m (ที่ x = {gsp.mu_pos_max_x_m:.2f} m)"},
    ]
    stir = gsp.stirrup
    shear = [
        {"desc": "กำลังรับแรงเฉือนของคอนกรีต V<sub>c</sub> (ช่วงวิกฤต)",
         "formula": "V<sub>c</sub> = 0.53√f'<sub>c</sub>·b·d", "sub": f"0.53×√{fc:.0f}×{b:.0f}×{gsp.bottom.d_cm:.1f}",
        ...
        {"desc": "แรงเฉือนที่เหล็กปลอกต้องรับ V<sub>s</sub>",
         ...
         "sub": f"{stir.vu_kg:,.0f}/{PHI_V:.2f} − {stir.vc_kg:,.0f}", "result": f"{stir.vs_req_kg:,.0f} kg"},
        {"desc": "ระยะเรียงเหล็กปลอกสูงสุด",
         "formula": "s<sub>max</sub> = min(A<sub>v</sub>·f<sub>y</sub>·d/V<sub>s</sub>, ระยะตามมาตรฐาน)",
         "result": f"{stir.s_max_cm:.1f} cm → ใช้ {inp.stirrup_legs}-{result.stirrup_bar_type}{inp.stirrup_bar_dia_mm:.0f}@{inp.stirrup_spacing_use_cm:.0f}cm"},
        {"desc": "ตรวจสอบเหล็กปลอก",
         "formula": f"ระยะที่ใช้ {inp.stirrup_spacing_use_cm:.0f} cm {'≤' if stir.stirrup_ok else '>'} s<sub>max</sub>",
         "result": "ผ่าน ✓" if stir.stirrup_ok else ("หน้าตัดเล็กเกินไป" if stir.section_too_small else "ไม่ผ่าน ✗")},
    ]
    ratios = [{"desc": "อัตราส่วนเหล็กเสริม (Reinforcement ratios)",
    ...
    bot_label = reinf_label_with_layers(gsp.bottom, result.main_bar_type, inp.main_bar_dia_mm)
    top_label = reinf_label_with_layers(gsup.top, result.main_bar_type, inp.main_bar_dia_mm)
    return [
        {"title": "การวิเคราะห์โครงสร้าง (Structural Analysis)", "steps": analysis + ratios},
        {"title": f"เหล็กเสริมล่างรับโมเมนต์บวก — ช่วงวิกฤตที่ {result.governing_span_index + 1} (Bottom Bars)",
         "steps": _flex_steps(gsp.bottom, gsp.mu_pos_max_kgm, bot_label, "ล่าง")},
        {"title": f"เหล็กเสริมบนรับโมเมนต์ลบ — จุดรองรับวิกฤต S{gsup.index} (Top Bars)",
         "steps": _flex_steps(gsup.top, abs(gsup.moment_kgm), top_label, "บน")},
        {"title": "การออกแบบเหล็กปลอกรับแรงเฉือน (Shear Design — ช่วงวิกฤต)", "steps": shear},
    ]
...
with st.container(border=True):
    ...
    # จุดรองรับปลายซ้าย/ขวา: เลือก Pin/Fix แยกกัน (แบบคานช่วงเดียว 3.1)
    _sup_opts = ["Pin (ยึดหมุน)", "Fix (ยึดแน่น)"]
    _es1, _es2 = st.columns(2)
    with _es1:
        _lsup = st.selectbox(
            "จุดรองรับปลายซ้าย (จุดแรก)", _sup_opts,
            index=1 if _loaded.get("left_end_fixed", _loaded.get("fixed_end_supports", True)) else 0,
            key=f"cb_lsup_{gen}")
    with _es2:
        _rsup = st.selectbox(
            "จุดรองรับปลายขวา (จุดสุดท้าย)", _sup_opts,
            index=1 if _loaded.get("right_end_fixed", _loaded.get("fixed_end_supports", True)) else 0,
            key=f"cb_rsup_{gen}")
    left_end_fixed = _lsup.startswith("Fix")
    right_end_fixed = _rsup.startswith("Fix")
    st.caption("Fix = ยึดแน่น เกิดโมเมนต์ลบที่ปลาย ออกแบบเหล็กบนที่ปลายได้จริง — "
               "ปลายด้านที่มีคานยื่นจะใช้โมเมนต์คานยื่นเสมอ (ไม่ขึ้นกับตัวเลือกนี้)")
...
inp = ContinuousBeamInput(
    ...
    left_end_fixed=left_end_fixed, right_end_fixed=right_end_fixed,
    ...
    with _tc2:
        tu_kgm = st.number_input("แรงบิดประลัย Tu (kg·m)", min_value=0.0,
                                 ...
                                 help="แรงบิดประลัย (factored) — คำนวณเหล็กปลอกและเหล็กยืนรับแรงบิดตาม ACI 318 "
                                      "(ออกแบบตามแรงเฉือน/หน้าตัดของแต่ละช่วงที่เลือก)")
    # เลือกช่วงที่มีแรงบิดกระทำ (เลือกได้มากกว่า 1 ช่วง) — แต่ละช่วงออกแบบตามแรงเฉือน/หน้าตัดของช่วงนั้น
    _span_labels = [f"ช่วงที่ {i + 1}" for i in range(len(span_inputs))]
    _saved_tu_spans = _loaded.get("tu_spans")
    if _saved_tu_spans:
        _default_tu = [f"ช่วงที่ {i + 1}" for i in _saved_tu_spans if 0 <= i < len(span_inputs)]
    else:
        _default_tu = _span_labels[:1]   # ค่าเริ่มต้น: ช่วงที่ 1
    _tu_span_labels = st.multiselect(
        "ช่วงที่มีแรงบิด Tu (เลือกได้มากกว่า 1 ช่วง)", _span_labels, default=_default_tu,
        key=f"cb_tuspans_{gen}", disabled=not torsion_enabled,
        help="เลือกช่วงคานที่มีแรงบิด Tu กระทำ — โปรแกรมจะออกแบบเหล็กรับแรงบิดแยกแต่ละช่วง "
             "ตามแรงเฉือน Vu และหน้าตัดของช่วงนั้น")
    tu_spans = [int(lbl.rsplit(" ", 1)[-1]) - 1 for lbl in _tu_span_labels]
...
with bcol1:
    if st.button("🧮 คำนวณ (Compute)", key="npk-btn-compute-cbeam", type="primary", use_container_width=True):
        ...
        if bad:
            ...
            st.session_state["cbeam_torsion_enabled"] = bool(torsion_enabled)
            st.session_state["cbeam_tu_kgm"] = float(tu_kgm)
            st.session_state["cbeam_tu_spans"] = list(tu_spans)
...
if "cbeam_result" in st.session_state:
    ...
    if result.left_fixed or result.right_fixed:
        ...
        st.info(f"🔩 จุดรองรับ {' และ '.join(_ends)} = FIX (ยึดแน่น) — "
    ...
    _tor = None   # torsion ของช่วงวิกฤต (governing) — ใช้กับรูปตัดที่วาดแสดง
    _tu_spans = [s for s in st.session_state.get("cbeam_tu_spans", []) if 0 <= s < len(result.spans)]
    if st.session_state.get("cbeam_torsion_enabled") and st.session_state.get("cbeam_tu_kgm", 0.0) > 0:
        if not _tu_spans:
            st.warning("เปิดออกแบบรับแรงบิดแล้ว แต่ยังไม่ได้เลือกช่วงที่มีแรงบิด — กรุณาเลือกช่วงในตัวเลือกด้านบนแล้วคำนวณใหม่")
        else:
            ...
            _tu = st.session_state["cbeam_tu_kgm"]
            st.subheader(f"ออกแบบรับแรงบิด (Torsion) — Tu = {_tu:,.0f} kg·m")
            for _si in sorted(_tu_spans):
                _sp = result.spans[_si]
                _tori = design_beam_torsion(
                    _tu, _sp.stirrup.vu_kg, inp.b_cm, inp.h_cm, _sp.bottom.d_cm, inp.fc_ksc,
                    _fyv, _fyl, _BEAM_COVER, inp.stirrup_bar_dia_mm, inp.stirrup_legs,
                    _sp.stirrup.vc_kg, _sp.stirrup.av_cm2, inp.stirrup_spacing_use_cm)
                st.markdown(f"**ช่วงที่ {_si + 1}** (Vu = {_sp.stirrup.vu_kg:,.0f} kg)")
                metric_card_row([
                    {"name": "แรงบิด Tu", "sym": "Torsion", "value": f"{_tori.tu_kgm:,.0f}", "unit": "kg·m", "ok": None},
                    {"name": "ต้องออกแบบแรงบิด?", "sym": f"เกณฑ์ {_tori.tth_kgm:,.0f}", "value": "ใช่" if _tori.required else "ไม่",
                     "unit": "kg·m", "ok": None},
                    {"name": "เหล็กปลอกรับบิด At/s", "sym": "ต่อขา", "value": f"{_tori.at_s:.4f}", "unit": "cm²/cm",
                     "ok": _tori.stirrup_ok if _tori.required else None, "reason": f"@{_tori.s_required_cm:.0f}cm" if _tori.required else "-"},
                    {"name": "เหล็กยืนรับบิด Al", "sym": "รวมรอบรูป", "value": f"{_tori.al_design_cm2:.2f}", "unit": "cm²",
                     "ok": _tori.section_ok if _tori.required else None, "reason": "หน้าตัดพอ" if _tori.section_ok else "หน้าตัดเล็กไป"},
                ])
                st.write("")
                _tsec = dict(build_torsion_section(_tori, result.stirrup_bar_type))
                _tsec["title"] = f"{_tsec['title']} — ช่วงที่ {_si + 1}"
                _calc_sections = _calc_sections + [_tsec]
                if _si == result.governing_span_index:
                    _tor = _tori   # ช่วงวิกฤตถูกเลือก → แสดงเหล็กรับบิดในรูปตัดกลางคานด้วย

    # --- การโก่งตัว (Deflection) ตามมาตรฐาน ACI: service + Ie (Branson) + long-term —
    #     คานต่อเนื่อง: y=0 ที่ทุกจุดรองรับ, ใช้หน้าตัด/โหลดของช่วงวิกฤต (governing span) เป็นตัวแทน ---
    _gspi = result.governing_span_index
    _gsp = result.spans[_gspi]
    _gin = inp.spans[_gspi]
    _w_dl_s = _gin.line_load_dl_kg_m + _gsp.self_weight_kg_m
    _w_ll_s = _gin.line_load_ll_kg_m
    _r_service = ((_w_dl_s + _w_ll_s) / _gsp.wu_kg_m) if _gsp.wu_kg_m > 0 else 0.0
    _sustained_frac = (_w_dl_s / (_w_dl_s + _w_ll_s)) if (_w_dl_s + _w_ll_s) > 0 else 0.0
    _as_comp = _gsp.bottom.as_comp_req_cm2 if _gsp.bottom.doubly_reinforced else 0.0
    _dpkg = compute_service_deflection(
        result.x_arr_full, result.m_arr_full, inp.b_cm, inp.h_cm, _gsp.bottom.d_cm, inp.fc_ksc,
        _gsp.bottom.as_provided_cm2, _as_comp, _gsp.mu_pos_max_kgm,
        r_service=_r_service, sustained_frac=_sustained_frac, span_L_m=_gsp.length_m,
        support_xs=list(result.support_x_positions), allow_ratio=360.0)
    # ปรับ L ในเกณฑ์ L/360 ให้เป็นช่วง/ปลายยื่นที่จุดโก่งสูงสุดตกอยู่จริง
    _sx = list(result.support_x_positions)
    _dx = _dpkg["imm_x"]
    if result.left_overhang is not None and _dx < _sx[0]:
        _seg_L = result.left_overhang.length_m
    elif result.right_overhang is not None and _dx > _sx[-1]:
        _seg_L = result.right_overhang.length_m
    else:
        _seg_L = _gsp.length_m
        for _i in range(len(_sx) - 1):
            if _sx[_i] <= _dx <= _sx[_i + 1]:
                _seg_L = result.spans[_i].length_m
                break
    _dpkg["span_L_m"] = _seg_L
    _dpkg["allow_mm"] = _seg_L * 1000.0 / 360.0
    _dpkg["ok"] = _dpkg["total_max"] <= _dpkg["allow_mm"]
    ...
    st.subheader("กราฟแรงเฉือน & โมเมนต์ & การโก่งตัวรวมทั้งคาน (SFD / BMD / Deflection)")
    sfd_bmd_png = draw_continuous_beam_sfd_bmd_png(
        result, defl_mm=_dpkg["total_arr"], defl_max_mm=_dpkg["total_max"], defl_max_x_m=_dpkg["imm_x"])
    ...
    support_section_png = draw_beam_section_png(
        ...
        inp.stirrup_bar_dia_mm, inp.stirrup_spacing_use_cm, result.stirrup_bar_type,
        torsion=_tor)
    # แสดงรูปตัด 2 ตำแหน่ง "ในแนวเดียวกัน" (คู่กัน 2 คอลัมน์) แบบเดียวกับในรายการคำนวณ (report .row3)
    # แต่ละรูปจัดกึ่งกลางในคอลัมน์ของตัวเอง (centered_image + max-width:100% พอดีความกว้างคอลัมน์)
    _sec_c1, _sec_c2 = st.columns(2)
    with _sec_c1:
        st.markdown(f"<div style='text-align:center;font-weight:bold;'>กลางคาน (Midspan) — ช่วงที่ {gsp_i + 1}</div>",
                    unsafe_allow_html=True)
        centered_image(midspan_section_png)
    with _sec_c2:
        st.markdown(f"<div style='text-align:center;font-weight:bold;'>จุดรองรับ (Support) — S{gsup_i}</div>",
                    unsafe_allow_html=True)
        centered_image(support_section_png)
    ...
    # --- ทางเลือก: จัดเหล็กเสริมเอง (Manual Rebar Arrangement) — 2 หน้าตัด (กลางคาน + จุดรองรับ)
    #     วางข้างกัน (ตารางบน รูปตัดล่าง) + ย่อรูปตัดให้กระชับพอดีหน้าเดียว ---
    ...
    with st.container(key="npk-manual-rebar"):
        with st.expander("🔧 จัดเหล็กเสริมเอง (Manual Rebar Arrangement)", expanded=False):
            st.caption("ปรับจำนวน/ขนาดเหล็กแต่ละชั้น (Main / Extra) เอง — คำนวณ As,use เทียบ As,req (OK/NG) "
                       "และวาดรูปตัดใหม่ตามที่จัด (ค่าชั่วคราว ไม่บันทึกลงโปรเจกต์)")
            _nb = result.nominal_bars
            _mcol1, _mcol2 = st.columns(2)
            with _mcol1:
                _mmid = manual_beam_section(
                    "cb_manual_mid", f"กลางคาน (Midspan) — ช่วงที่ {gsp_i + 1}",
                    as_req_top=_nb.as_req_cm2, as_req_bot=gsp.bottom.as_req_cm2,
                    default_top=_nb.bars_per_layer, default_bot=gsp.bottom.bars_per_layer,
                    ...
                    s_use_default=inp.stirrup_spacing_use_cm, s_max_cm=gsp.stirrup.s_max_cm, gen=gen, torsion=_tor,
                    stirrup_sizes=STIRRUP_DIAMETERS_MM)
                centered_image(_mmid["png"], caption="Midspan Section (Manual)")
            with _mcol2:
                _msup = manual_beam_section(
                    "cb_manual_sup", f"จุดรองรับ (Support) — S{gsup_i}",
                    as_req_top=gsup.top.as_req_cm2, as_req_bot=_nb.as_req_cm2,
                    default_top=gsup.top.bars_per_layer, default_bot=_nb.bars_per_layer,
                    b_cm=inp.b_cm, h_cm=inp.h_cm, main_bar_type=result.main_bar_type, main_dia=inp.main_bar_dia_mm,
                    bar_sizes=BAR_DIAMETERS_MM, stirrup_dia=inp.stirrup_bar_dia_mm, stirrup_type=result.stirrup_bar_type,
                    s_use_default=inp.stirrup_spacing_use_cm, s_max_cm=gsp.stirrup.s_max_cm, gen=gen, torsion=_tor,
                    stirrup_sizes=STIRRUP_DIAMETERS_MM)
                centered_image(_msup["png"], caption="Support Section (Manual)")
