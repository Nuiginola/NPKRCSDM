...
class ContinuousBeamInput:
    ...
    # จุดรองรับปลาย (จุดแรก/จุดสุดท้าย) เลือก Pin/Fix แยกแต่ละปลายได้ (แบบคานช่วงเดียว 3.1)
    # — เป็น FIX เฉพาะเมื่อด้านนั้นไม่มีคานยื่น (ด้านที่มีคานยื่นใช้โมเมนต์คานยื่นเสมอ ไม่กระทบ)
    left_end_fixed: bool = True
    right_end_fixed: bool = True
    # (เดิม) สวิตช์รวมทั้งสองปลาย — คงไว้เพื่อความเข้ากันได้กับรายการที่บันทึกไว้ก่อนหน้า (ไม่ใช้ในการคำนวณแล้ว)
    fixed_end_supports: bool = True
    # โหมดจัดเหล็กเอง (ทั้งคาน) : ระบุจำนวนเส้น เหล็กล่าง/บน แถวที่ 1/2 เอง แทนการออกแบบอัตโนมัติ
    manual_reinf: bool = False
    manual_bot_row1: int = 0
    manual_bot_row2: int = 0
    manual_top_row1: int = 0
    manual_top_row2: int = 0
...
class ContinuousBeamResult:
    ...
    # จุดรองรับปลาย FIX
    fixed_end_supports: bool = False
    left_fixed: bool = False
    right_fixed: bool = False
    # โหมดจัดเหล็กเอง
    manual_reinf: bool = False
    manual_bottom: object = None          # FlexureDesign ของเหล็กล่างที่จัดเอง
    manual_top: object = None             # FlexureDesign ของเหล็กบนที่จัดเอง
    manual_req_bottom_cm2: float = 0.0    # As เหล็กล่างที่ต้องการ (governing) สำหรับเทียบ
    manual_req_top_cm2: float = 0.0
    design_ok: bool = True                # ผลตรวจสอบรวม (โหมดจัดเอง: บล็อกถ้าเหล็กไม่พอ)
...
def calculate(inp: ContinuousBeamInput) -> ContinuousBeamResult:
    ...
    # --- จุดรองรับปลายเป็น FIX (เมื่อเปิดสวิตช์ และด้านนั้นไม่มีคานยื่น) ---
    # จำลองด้วยเทคนิคช่วงจินตภาพยาว 0 (imaginary zero-length span) ที่ด้านนอกของปลาย
    # -> บังคับมุมหมุน = 0 พอดี ; โมเมนต์ปลายกลายเป็นตัวไม่ทราบค่าเพิ่มในระบบสมการ
    left_fixed = bool(inp.left_end_fixed) and (left_ov is None)
    right_fixed = bool(inp.right_end_fixed) and (right_ov is None)

    # --- แก้ระบบสมการ Three Moment Equation ---
    M = [0.0] * (N + 1)   # M[0..N] ที่จุดรองรับ 0..N (0=ซ้ายสุด, N=ขวาสุด)
    M[0] = m0_known       # ค่าเริ่มต้น (pinned=0 หรือโมเมนต์คานยื่น) — ถูกแทนถ้าปลายเป็น FIX
    M[N] = mN_known

    # จุดรองรับที่เป็น "ตัวไม่ทราบค่า": ปลายซ้าย(ถ้า FIX) + ภายใน 1..N-1 + ปลายขวา(ถ้า FIX)
    unknown = ([0] if left_fixed else []) + list(range(1, N)) + ([N] if right_fixed else [])
    ...
    if nu > 0:
        ...
        for k, s in enumerate(unknown):
            if s == 0:
                # ปลายซ้าย FIX : 2*M0*L1 + M1*L1 = -term_L(ช่วงแรก)
                L1 = lengths[0]
                A[k, pos[0]] += 2.0 * L1
                if 1 in pos:
                    A[k, pos[1]] += L1
                ...
                # ปลายขวา FIX : M(N-1)*LN + 2*MN*LN = -term_R(ช่วงสุดท้าย)
                LN = lengths[N - 1]
                A[k, pos[N]] += 2.0 * LN
                if (N - 1) in pos:
                    A[k, pos[N - 1]] += LN
                    ...
                    rhs[k] -= M[N - 1] * LN
                rhs[k] += -term_R_list[N - 1]
            else:
                # จุดรองรับภายใน s (1..N-1)
                L_i = lengths[s - 1]
                L_ip1 = lengths[s]
                A[k, pos[s]] += 2.0 * (L_i + L_ip1)
                if (s - 1) in pos:
                    A[k, pos[s - 1]] += L_i
                else:
                    rhs[k] -= M[s - 1] * L_i
                if (s + 1) in pos:
                    ...
                    rhs[k] -= M[s + 1] * L_ip1
                rhs[k] += -(term_R_list[s - 1] + term_L_list[s])
    ...
    # --- โหมดจัดเหล็กเอง : ตรวจสอบว่า As ที่จัดเองพอกับ As ที่ต้องการ (governing) หรือไม่ ---
    manual_bottom = manual_top = None
    manual_req_bottom = manual_req_top = 0.0
    design_ok = True
    design_fail_reason = ""

    def _manual_flex(row1, row2, as_req):
        n_total = int(row1) + int(row2)
        bar_a = bar_area_cm2(inp.main_bar_dia_mm)
        as_prov = n_total * bar_a
        bpl = [r for r in [int(row1), int(row2)] if r > 0] or [0]
        max_per = _max_bars_single_layer(inp.b_cm, inp.main_bar_dia_mm, inp.stirrup_bar_dia_mm)
        fits = all(r <= max_per for r in bpl)
        ok = (n_total > 0) and (as_prov >= as_req - 1e-6) and fits
        d_cm = inp.h_cm - COVER_CM - inp.stirrup_bar_dia_mm / 10.0 - inp.main_bar_dia_mm / 20.0
        fd = FlexureDesign(
            mu_kgm=0.0, d_cm=d_cm, ru_ksc=0.0, doubly_reinforced=False,
            mu1_kgm=0.0, mu2_kgm=0.0, rreq=0.0, rho_used=0.0,
            as_req_cm2=as_req, as_comp_req_cm2=0.0,
            n_bars_req=int(math.ceil(as_req / bar_a)) if bar_a > 0 else 0,
            n_bars_use=n_total, as_provided_cm2=as_prov,
            fits_single_layer=(len(bpl) == 1 and fits), max_bars_single_layer=max_per,
            reinf_ok=ok, n_layers=len(bpl), bars_per_layer=bpl,
        )
        return fd, ok, fits, n_total

    if inp.manual_reinf:
        manual_req_bottom = max((s.bottom.as_req_cm2 for s in spans_out), default=0.0)
        tops = [s.top.as_req_cm2 for s in supports_out]
        if left_ov_out is not None:
            tops.append(left_ov_out.top.as_req_cm2)
        if right_ov_out is not None:
            tops.append(right_ov_out.top.as_req_cm2)
        manual_req_top = max(tops, default=0.0)
        manual_bottom, ok_b, fit_b, nb = _manual_flex(inp.manual_bot_row1, inp.manual_bot_row2, manual_req_bottom)
        manual_top, ok_t, fit_t, nt = _manual_flex(inp.manual_top_row1, inp.manual_top_row2, manual_req_top)
        design_ok = ok_b and ok_t
        reasons = []
        if not ok_b:
            if nb <= 0:
                reasons.append("ยังไม่ได้ระบุจำนวนเหล็กล่าง")
            elif not fit_b:
                reasons.append("เหล็กล่างจัดไม่ลงในความกว้างคาน (จำนวนต่อแถวเกินค่าสูงสุด)")
            else:
                reasons.append(f"เหล็กล่างไม่พอ (As จัดได้ {manual_bottom.as_provided_cm2:.2f} < ต้องการ {manual_req_bottom:.2f} cm²)")
        if not ok_t:
            if nt <= 0:
                reasons.append("ยังไม่ได้ระบุจำนวนเหล็กบน")
            elif not fit_t:
                reasons.append("เหล็กบนจัดไม่ลงในความกว้างคาน (จำนวนต่อแถวเกินค่าสูงสุด)")
            else:
                reasons.append(f"เหล็กบนไม่พอ (As จัดได้ {manual_top.as_provided_cm2:.2f} < ต้องการ {manual_req_top:.2f} cm²)")
        design_fail_reason = " ; ".join(reasons)
    ...
    return ContinuousBeamResult(
        ...
        fixed_end_supports=bool(left_fixed or right_fixed),
        left_fixed=left_fixed, right_fixed=right_fixed,
        manual_reinf=bool(inp.manual_reinf),
        manual_bottom=manual_bottom, manual_top=manual_top,
        manual_req_bottom_cm2=manual_req_bottom, manual_req_top_cm2=manual_req_top,
