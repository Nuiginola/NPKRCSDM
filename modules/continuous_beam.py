...
class ContinuousBeamInput:
    ...
    # จุดรองรับปลาย (จุดแรก/จุดสุดท้าย) เป็นแบบ FIX เมื่อด้านนั้นไม่มีคานยื่น — ออกแบบเหล็กบนที่
    # ปลายได้จริง (เปิด/ปิดได้ ; ด้านที่มีคานยื่นยังใช้โมเมนต์คานยื่นเสมอ ไม่กระทบ)
...
def calculate(inp: ContinuousBeamInput) -> ContinuousBeamResult:
    ...
    left_fixed = bool(inp.fixed_end_supports) and (left_ov is None)
    right_fixed = bool(inp.fixed_end_supports) and (right_ov is None)
    ...
    return ContinuousBeamResult(
        ...
        fixed_end_supports=bool(inp.fixed_end_supports),
