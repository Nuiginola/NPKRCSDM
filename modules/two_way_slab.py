...
============================================================================
CORRECTION — 2026-07-12, based on user-provided reference images
(project folder "Slapcase", 5 PNGs, one per case) that were NOT available
when the above analysis was originally written
============================================================================
The user's own reference drawings (one detailed rebar-layout drawing per
case, showing the negative-steel cut-off length at all 4 edges — the same
"span/3 continuous vs. span/4 discontinuous" convention documented above)
reveal that CASE3 and CASE4's continuous/discontinuous EDGE ASSIGNMENT per
direction, as shipped before this date, was topologically wrong — i.e. the
case-to-case mapping guess described above (paragraph starting "The one
genuinely independent cross-check...") got CASE3 and CASE4 wrong. Verified
directly against the reference drawings (not guessed):

  - CASE3 ("ไม่ต่อเนื่อง 2 ด้าน"): the reference shows the SHORT direction
    fully continuous at BOTH edges (0.83=S/3 cut-off at both #1 and #3) and
    the LONG direction fully discontinuous at BOTH edges (0.75=L/4 cut-off
    at both #4 and #6) — i.e. the two discontinuous edges are the two
    OPPOSITE/parallel long-direction edges, NOT two adjacent edges meeting
    at one corner as the old label_th ("พื้นมุม") assumed. Old code had
    BOTH directions as "mixed" (has_disc, one continuous + one
    discontinuous edge each) — wrong for both directions.
  - CASE4 ("ไม่ต่อเนื่อง 3 ด้าน"): the reference shows the SHORT direction
    MIXED (one continuous edge at 0.83=S/3, one discontinuous at
    0.63=S/4) and the LONG direction fully discontinuous at BOTH edges
    (0.75=L/4 at both #4 and #6). Old code had it backwards: short fully
    discontinuous (con- forced to zero — meaning the real continuous edge's
    negative-moment top steel was simply never designed), long mixed.

Fix applied: swapped each direction's edge-condition TYPE (fully-continuous
/ mixed / fully-discontinuous) to match the reference drawings exactly.
For the actual NUMERIC coefficient arrays needed to fill in the corrected
topology, no confirmed source table was available for these specific
"opposite-edges-discontinuous" sub-cases (same known gap as "cases 6-9"
already flagged above), so each corrected direction's array was borrowed
from whichever OTHER already-existing case in this same table shares the
identical edge-condition type for that direction (fully-cont ↔ fully-cont,
mixed ↔ mixed, fully-disc ↔ fully-disc), on the standard simplifying
assumption that a direction's own con-/mid+ coefficients depend mostly on
that direction's own edge condition and m, with the other direction's
condition being a secondary effect:
  - CASE3 short (fully continuous both edges) ← reused CASE1 short's con-
    array (CASE1 short is also "fully continuous both edges").
  - CASE3 long (fully discontinuous both edges) ← reused CASE5 long's mid+
    array (CASE5 long is also "fully discontinuous both edges").
  - CASE4 short (mixed, 1 continuous + 1 discontinuous) ← reused CASE2
    short's con- array (CASE2 short is also "mixed, 1 continuous + 1
    discontinuous" — the one fully-confirmed mixed pattern in this table).
  - CASE4 long (fully discontinuous both edges) ← reused CASE5 long's mid+
    array (same reasoning as CASE3 long above).
This is a reasoned approximation, NOT a confirmed value — "confirmed"
stays False for both cases. Please verify against a proper textbook/your
advisor before relying on CASE3 or CASE4 for final thesis design; the
topology (which edge gets real top steel at all) is now solid, but the
exact magnitude may be off. CASE1, CASE2, and CASE5 themselves were not
touched by this correction (CASE1's own possible mislabeling — its numbers
matching a published "2 continuous + 2 discontinuous adjacent" table
almost exactly instead of a true "4 continuous / interior panel" table —
was raised to the user separately and is intentionally NOT auto-corrected
here pending their decision).
...
TWO_WAY_CASES = {
    ...
    "CASE3": {
        "label_th": "ไม่ต่อเนื่อง 2 ด้าน ตรงข้ามกัน (2 Edge Discontinuous, opposite — "
                    "ทิศทางยาวไม่ต่อเนื่องทั้ง 2 ขอบ, ทิศทางสั้นต่อเนื่องทั้ง 2 ขอบ) "
                    "[แก้ทิศทางขอบ 2026-07-12 ให้ตรงกับภาพอ้างอิงจริง — ดู docstring]",
        "confirmed": False,
        "short": {
            "has_disc": False,
            # ทั้ง 2 ขอบต่อเนื่อง (ยืนยันจากภาพอ้างอิง Slapcase) — ยืมตาราง con ของ CASE1
            # short (เป็น "ทิศทางสั้น ต่อเนื่องทั้ง 2 ขอบ" แบบเดียวกัน) ดู docstring
            "con": [0.094, 0.092, 0.089, 0.085, 0.081, 0.076, 0.071, 0.066, 0.060, 0.055, 0.050],
        },
        "long": {
            "has_disc": False,
            "fully_disc": True,   # ทั้ง 2 ขอบไม่ต่อเนื่อง — ไม่มีขอบต่อเนื่องให้คำนวณ Con.-
            "con": [0.0] * 11,
            # ยืมตาราง mid ของ CASE5 long (เป็น "ทิศทางยาว ไม่ต่อเนื่องทั้ง 2 ขอบ" แบบเดียวกัน)
            "mid": [0.006, 0.008, 0.010, 0.013, 0.016, 0.019, 0.023, 0.026, 0.029, 0.033, 0.036],
        },
    },
    "CASE4": {
        "label_th": "ไม่ต่อเนื่อง 3 ด้าน (3 Edge Discontinuous — ทิศทางสั้นต่อเนื่อง 1 ขอบ, "
                    "ทิศทางยาวไม่ต่อเนื่องทั้ง 2 ขอบ) "
                    "[แก้ทิศทางขอบ 2026-07-12 ให้ตรงกับภาพอ้างอิงจริง — ดู docstring]",
        "confirmed": False,
        "short": {
            "has_disc": True,   # ขอบหนึ่งต่อเนื่อง + ขอบหนึ่งไม่ต่อเนื่อง (ยืนยันจากภาพ)
            # ยืมตาราง con ของ CASE2 short (เป็น "ทิศทางสั้น ผสม 1 ต่อเนื่อง+1 ไม่ต่อเนื่อง" แบบเดียวกัน)
            "con": [0.085, 0.077, 0.069, 0.0655, 0.062, 0.0585, 0.055, 0.0515, 0.048, 0.0445, 0.041],
        },
        "long": {
            "has_disc": False,
            "fully_disc": True,   # ทั้ง 2 ขอบไม่ต่อเนื่อง (ยืนยันจากภาพ)
            "con": [0.0] * 11,
            # ยืมตาราง mid ของ CASE5 long (เหตุผลเดียวกับ CASE3 long ด้านบน)
            "mid": [0.006, 0.008, 0.010, 0.013, 0.016, 0.019, 0.023, 0.026, 0.029, 0.033, 0.036],
        },
    },
    "CASE5": {
...
def _force_min_top_steel_at_disc_edges(positions: list, case_dir: dict, ast_min: float) -> list:
    """บังคับเหล็กบนขั้นต่ำที่ "ขอบไม่ต่อเนื่อง" (Disc.-) เสมอ — ใช้กับกรณี 2-5 ทุกกรณี
    (ผู้ใช้สั่งเป็นข้อบังคับ 2026-07-12; ตรงกับข้อกำหนด ACI 318 ที่ขอบไม่ต่อเนื่องต้องมี
    เหล็กบนกันการแตกร้าว/โมเมนต์บิดปลาย ไม่ปล่อยว่าง แม้ตารางสัมประสิทธิ์จะให้ค่า 0).

    เหล็กบนขั้นต่ำ = max( 1/3 ของเหล็กล่างช่วงกลาง (ตามข้อกำหนดขอบไม่ต่อเนื่อง ACI 318),
    Ast ขั้นต่ำ 0.002·b·t ). เปิดใช้งานตำแหน่ง Disc.- ให้ active เพื่อให้ทั้งรายงานและรูปวาด
    แสดงเหล็กบนที่ขอบไม่ต่อเนื่อง (ยื่นระยะ span/4) — ครอบคลุม 2 กรณีย่อย:
      (ก) ทิศทางไม่ต่อเนื่องทั้ง 2 ขอบ (fully_disc): เดิม Disc.- ไม่ active เลย (ตารางให้ 0)
          → เปิดใช้งานให้มีเหล็กบนขั้นต่ำที่ทั้ง 2 ขอบ
      (ข) ทิศทางผสม (มี Disc.- อยู่แล้วจากสัมประสิทธิ์): ยก As ขึ้นเป็นอย่างน้อยค่าขั้นต่ำนี้
    หมายเหตุ: ไม่กระทบ as_req รวมของทิศทาง (ซึ่ง = max ทุกตำแหน่ง แล้ว floor ด้วย ast_min อยู่แล้ว)
    เพราะ As ขั้นต่ำนี้ <= As ช่วงกลางเสมอ."""
    mid = positions[1]       # ตำแหน่ง Mid.+ (เหล็กล่างช่วงกลาง)
    disc = positions[2]      # ตำแหน่ง Disc.- (ขอบไม่ต่อเนื่อง)
    as_min_top = max(mid.as_req_cm2_m / 3.0, ast_min)

    is_fully_disc = case_dir.get("fully_disc", False)
    is_mixed_disc = case_dir.get("has_disc", False)
    if not (is_fully_disc or is_mixed_disc):
        return positions      # ทิศทางต่อเนื่องล้วน (ไม่มีขอบไม่ต่อเนื่อง) — ไม่ต้องทำอะไร

    if not disc.active:
        # กรณี (ก) fully_disc: สร้างตำแหน่ง Disc.- เป็นเหล็กบนขั้นต่ำ (Mu อ้างอิง 1/3 ช่วงกลาง)
        positions[2] = replace(
            disc, active=True, coeff=0.0, mu_kgm=mid.mu_kgm / 3.0,
            rho_used=as_min_top / (100.0 * disc.d_cm) if disc.d_cm else 0.0,
            over_reinforced=False, as_req_cm2_m=as_min_top,
        )
    elif disc.as_req_cm2_m < as_min_top:
        # กรณี (ข) mixed: ยก As ขอบไม่ต่อเนื่องขึ้นเป็นค่าขั้นต่ำ
        positions[2] = replace(disc, as_req_cm2_m=as_min_top)
    return positions
...
def calculate(inp: TwoWaySlabInput) -> TwoWaySlabResult:
    ...
    # --- บังคับเหล็กบนขั้นต่ำที่ขอบไม่ต่อเนื่องทุกกรณี (ผู้ใช้สั่ง 2026-07-12) ---
    # เปิดใช้งานตำแหน่ง Disc.- ให้แสดง/วาดเหล็กบนขั้นต่ำที่ขอบไม่ต่อเนื่องเสมอ
    # (ไม่กระทบ as_req รวมด้านบน เพราะ As ขั้นต่ำ <= As ช่วงกลางเสมอ)
    short_positions = _force_min_top_steel_at_disc_edges(short_positions, case["short"], ast_min)
    long_positions = _force_min_top_steel_at_disc_edges(long_positions, case["long"], ast_min)
